...
class MultiLayer:
    ...
    def loss_function(self, y_hat):
        ...
        # Added l2 regularization loss here
        w1_sq = np.sum(self.w1*self.w1)
        w2_sq = np.sum(self.w2*self.w2)
        sum_reg = w1_sq+w2_sq
        loss = ((-1 / (self.m * self.n)) * sum_m) + ((lam / (2 * self.n)) *sum_reg)
        return loss

    def backward(self, y_hat):
        # Calculate the derivative of w2 and b2
        reg_sum = (lam/self.m)*self.w2
        dw2 = (1 / self.m) * np.matmul(self.a1.T, (y_hat - self.y)) + reg_sum
        ...
        dz1 = da1 * dz1
        dw1 = (1 / self.m) * np.matmul(self.x.T, dz1) + ((lam/self.m)*self.w1)
